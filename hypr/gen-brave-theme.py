...
def main():
    ...
    # wants the browser's palette flipped (toolbar takes the secondary color,
    ...
    # version. This is what makes a CLOSED Brave pick up new colors: on the next
    # launch --load-extension only re-bakes the cached theme if the version
    # changed (when Brave's open, brave-theme-reload.py forces it via CDP, which
    # is why it always worked there). A crc32 over the full color set gives a
    # practically collision-free 32-bit version — the old sum-of-chars hash had
    # only ~1000 possible, order-independent values, so different wallpapers
    # routinely produced the SAME version and Chromium served the stale cache.
    seed = "".join("{:02x}{:02x}{:02x}".format(*c)
                   for c in (primary, secondary, accent, ink_p, ink_s))
    h = zlib.crc32(seed.encode()) & 0xFFFFFFFF
    manifest = {
        "manifest_version": 3,
        "name": "Technicolor",
        "version": "1.0.%d.%d" % (h >> 16, h & 0xFFFF),
