        ...
        try:
            ...
            with open(conf_path, 'w') as cf:
                cf.write("# Auto-generated by wallpaper-colors.py — focused-window border\n"
                         "# = the wallpaper's primary accent. Applied live + at startup.\n"
                         "general {\n    col.active_border = " + border + "\n}\n")
            # Apply live via `hyprctl eval` — `hyprctl keyword` is rejected under the
            # Lua config manager (that's why the border had gone white).
            subprocess.run(['hyprctl', 'eval',
                            'hl.config({general={col={active_border="' + border + '"}}}) return 1'],
                           capture_output=True)
        except Exception:
            pass
