...
if [[ "$XDG_CURRENT_DESKTOP" == *"KDE"* ]]; then
    ...
    if [ -n "$PREV_PATH" ] && [ -f "$PREV_PATH" ] && [ "$PREV_PATH" != "$CHOSEN" ]; then
        ...
        # the GPU — awww keeps animating OLD underneath, NEW animates natively
        # in the overlay, and the awww swap happens invisibly at the end. Both
        # wallpapers stay in motion, aspect mismatches crop instead of
        ...
        if [[ "$EXT" != "mp4" && "$EXT" != "webm" && "$TRENDER" != "cpu" ]]; then
            ...
            if [ -n "$META" ]; then
                ...
                WF_AVGMS=$(jq -r '.avgMs' <<<"$META" 2>/dev/null)
                # Bar/app colors cross-fade alongside the reveal (the old
        ...
        if [ -z "$WF_OK" ]; then
            ...
            if [ -n "${COLORS_RAN:-}" ]; then
                python3 "$HOME/.config/hypr/wallpaper-transition.py" \
                    "$PREV_PATH" "$CHOSEN" --mode "${TMODE:-luminance}"
            else
                python3 "$HOME/.config/hypr/wallpaper-transition.py" \
