...
## App theming (Discord / Spotify / Dolphin / Brave / GTK / LibreOffice)
...
**LibreOffice:** the hard one — and it splits in two, because LibreOffice gets colors from two different places. (1) The **chrome** (toolbars, menubar, sidebar, statusbar, ruler) is painted by the **kf6 VCL plugin from the Qt palette = `kdeglobals`**, which `gen-kde-colors.py` already rewrites from the wallpaper. So launch LibreOffice on kf6 with **`LibreOfficeTheme=0`** (theming off) and the whole UI follows the wallpaper for free — colored (sukapura) icons intact. This updates on LibreOffice **relaunch** (kf6 reads the palette at startup; there's no live hook — the same wall Dolphin needed an `LD_PRELOAD` shim to get around, which LibreOffice has no equivalent for). (2) The **document colors** — the page and the blank gutter around it — come from LibreOffice's own `ColorConfig`, *not* the Qt palette, so those ARE settable live, via **UNO** (the scripting bridge running inside the LibreOffice process). `lo-recolor.py` connects over UNO and forces the page (`DocColor`) **white** with black text, and sets the gutter (`AppBackground`) to a fixed key color so the [chromakey glass](#app-theming-discord--spotify--dolphin--brave--gtk--libreoffice) layer can make the blank area around the page **liquid glass** (it reuses Dolphin's `tckeydolph` shader — same key). `wallpaper-colors.py` calls it on each wallpaper change.

Setup: (1) it needs the LibreOffice python's `uno` module — on Arch that's whatever interpreter owns `…/site-packages/uno.py` (here `python3.14`; the script shebang points at it). (2) UNO only listens if LibreOffice launched with an acceptor socket, and the chrome only follows the wallpaper on the kf6 plugin — `hypr/lo-tc.sh` handles both (`export SAL_USE_VCLPLUGIN=kf6`, adds `--accept=socket,...:2002`, fires the recolor once the socket is up). Point your `.desktop` launchers at it:
```
mkdir -p ~/.local/share/applications
for v in writer calc impress draw math base startcenter; do
  sed -E "s#^Exec=(/usr/bin/)?libreoffice ?#Exec=$HOME/.config/hypr/lo-tc.sh #" \
    /usr/share/applications/libreoffice-$v.desktop > ~/.local/share/applications/libreoffice-$v.desktop 2>/dev/null
done
update-desktop-database ~/.local/share/applications 2>/dev/null
```
Set a colored icon theme in Tools → Options → LibreOffice → View → Icon style → **Sukapura** (or Karasa Jaga). For the glass gutter, add the LibreOffice `darkwindow:shade` windowrules from `local.conf.example`. (The `.desktop` overrides hold absolute paths so they're not committed — regenerate with the snippet above. What LibreOffice *can't* do: live chrome recolor while open, palette-colored toolbars via its own scheme on kf6, or rounded toolbars — those are VCL limitations.)
