...
**LibreOffice:** the hard one. LibreOffice draws its own UI with its in-house VCL toolkit, so — unlike Dolphin — it ignores `kdeglobals`/the Qt palette at runtime; no external signal recolors it. The lever that *does* reach its in-memory color cache is **UNO**, the scripting bridge that runs inside the LibreOffice process. `lo-recolor.py` connects over UNO and rewrites a named color scheme ("Technicolor") live: the chrome (toolbars, menus, the gutter around the page) takes the wallpaper palette, while the document page is forced **white** with black text (so writing stays readable). It's wired in as the `libreoffice` [per-surface](#settings-app) tuning target, so its saturation/brightness/hue sliders work like every other app. `wallpaper-colors.py` calls it on every wallpaper change, so an already-open LibreOffice recolors live. Two setup pieces: (1) it needs the LibreOffice python's `uno` module — on Arch that's whatever interpreter owns `…/site-packages/uno.py` (here `python3.14`; the script's shebang points at it — adjust if your LO ships a different python). (2) UNO only listens if LibreOffice was launched with an acceptor socket, so launch it through `hypr/lo-tc.sh` — it adds `--accept=socket,...:2002`, passes through all args, and fires the recolor once the socket is up. Point your LibreOffice `.desktop` launchers at it:
```
...
```
(The generated overrides hold absolute paths, so they're not committed — regenerate them with the snippet above. Revert LibreOffice to stock by deleting them and setting `LibreOfficeTheme=0` in Tools → Options → LibreOffice → Appearance.)
